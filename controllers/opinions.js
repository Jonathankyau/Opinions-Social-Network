...
module.exports = {
    getOpinions: async (req,res)=>{
        ...
        try{
            ...
            const itemsLeft = await Opinion.countDocuments({userId:req.user.id,completed: false})
            res.render('opinions.ejs', {opinions: opinionItems, left: itemsLeft, user: req.user})
        }catch(err){
            console.log(err)
        }
    },
    createOpinion: async (req, res)=>{
        try{
            await Opinion.create({opinion: req.body.opinionItem, completed: false, userId: req.user.id})
